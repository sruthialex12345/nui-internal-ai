...
export function devMetricsPlugin(options = {}) {
    const { enabled = true, showStartupTime = true, showBuildInfo = true, trackHMR = true, appName = 'App' } = options;
    if (!enabled)
        return { name: 'dev-metrics', apply: 'serve' };
    let serverStartTime;
    let hmrCount = 0;
    let lastHMRTime = 0;
    return {
                ...
                if (showStartupTime) {
                    setTimeout(() => {
                        ...
                        if (startupTime < 1000) {
                            ...
                            console.log('⚠️  Status: Moderate (3-5s)');
                        }
                        else {
                            console.log('❌ Status: Slow (>5s) - Check for optimization opportunities');
                        }
                        console.log('');
                    }, 100);
                }
