...
export interface PerformanceThresholds {
  viteStartup: {
    excellent: number;  // < 1s
    good: number;       // 1-3s
    moderate: number;   // 3-5s
    // > 5s = slow
  };
  typescript: {
    excellent: number;  // < 5s
    good: number;       // 5-15s  
    moderate: number;   // 15-30s
    // > 30s = slow
  };
  eslint: {
    excellent: number;  // < 3s
    good: number;       // 3-10s
    moderate: number;   // 10-20s
    // > 20s = slow
  };
  build: {
    excellent: number;  // < 10s
    good: number;       // 10-30s
    moderate: number;   // 30-60s
    // > 60s = slow
  };
}
...
export interface MetricsRun {
  ...
  appName: string;
  viteStartup?: number;
  typescript?: { time: number; success: boolean };
  eslint?: { time: number; success: boolean };
  build?: { time: number; success: boolean };
  stats?: {
    totalFiles: number;
    tsFiles: number;
    linesOfCode: number;
...
export class DevMetrics {
  ...
  async measureViteStartup(): Promise<number> {
  ...
  async measureTypeScript(): Promise<{ time: number; success: boolean }> {
  ...
  async measureESLint(): Promise<{ time: number; success: boolean }> {
  ...
  async measureBuild(): Promise<{ time: number; success: boolean }> {
  ...
  calculateAverages() {
    if (this.metrics.runs.length === 0) return {};

    const runs = this.metrics.runs.filter((run: MetricsRun) => run.appName === this.config.appName);
    
    return {
      viteStartup: runs.reduce((sum: number, run: MetricsRun) => sum + (run.viteStartup || 0), 0) / runs.length,
      typescript: runs.reduce((sum: number, run: MetricsRun) => sum + (run.typescript?.time || 0), 0) / runs.length,
      eslint: runs.reduce((sum: number, run: MetricsRun) => sum + (run.eslint?.time || 0), 0) / runs.length,
      build: runs.reduce((sum: number, run: MetricsRun) => sum + (run.build?.time || 0), 0) / runs.length,
    };
  }

  private assessPerformance(metric: 'viteStartup' | 'typescript' | 'eslint' | 'build', value: number): {
  ...
  private getRecommendations(run: MetricsRun): string[] {
  ...
  async runFullAnalysis(): Promise<MetricsRun> {
    ...
    const appRuns = this.metrics.runs.filter((r: MetricsRun) => r.appName === this.config.appName);
    if (appRuns.length > 10) {
      this.metrics.runs = this.metrics.runs.filter((r: MetricsRun) => 
        r.appName !== this.config.appName
      ).concat(appRuns.slice(-10));
    }
  ...
  printReport(run: MetricsRun) {
    ...
    if (viteAssessment.isWarning) hasWarnings = true;
    console.log('');
    ...
    if (tsAssessment.isWarning) hasWarnings = true;
    console.log('');
    ...
    if (eslintAssessment.isWarning) hasWarnings = true;
    console.log('');
    ...
    if (buildAssessment.isWarning) hasWarnings = true;
    console.log('');
